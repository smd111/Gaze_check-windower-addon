...
function check_facing(packet)
    ...
    for i,v in pairs(packet) do
        if angle.player < 90 and angle.actor < 90 then
            return true
        elseif string.match(i, 'Target %d+ Action %d+ Param') then --Turn on gazes than don't need the mob to face you to apply
            if T{1694, 1695, 1713, 1716, 1762, 1950, 1978, 2155, 2814, 2817}:contains(v) and angle.player < 90 then
                return true
            end
        end
    end
